...
#   退出中や初参加のプレイヤーは .teamHP を上書きしないようにする
...
# sharedHealth :
#   チームの共有体力 (.teamHP_red | .teamHP_blue)
#   同じスコアボードで、異なる仮想プレイヤー名 (.teamHP_red | .teamHP_blue) でチームを区別する
...
#   この状態のプレイヤーは .teamHP への書き込みを遮断される
...
# Tick Log Trigger
scoreboard objectives add logTrigger dummy
scoreboard players set #DEBUG logTrigger 1

# ======
say team_shared_hp loaded
# ======
